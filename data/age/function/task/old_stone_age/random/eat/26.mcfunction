...
execute unless entity @s[tag=task_done] unless score @s seed_eat = @s seed_updata run title @s actionbar ["",{"text":"🍖 ","color":"#55FF55"},{"text":"食用: 种子 ","color":"#55FF55"},{"score":{"name":"@s","objective":"seed_eat"},"color":"gray"},{"text":"/8","color":"gray"}]
...
execute as @s unless entity @s[tag=task_done] if score @s seed_eat matches 8.. run function age:task/old_stone_age/random/eat/26_c